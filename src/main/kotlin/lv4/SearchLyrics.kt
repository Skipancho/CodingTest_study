...
class SearchLyrics {
    ...
    fun solution(words: Array<String>, queries: Array<String>): IntArray {
        ...
        for (word in words){
            ...
            if (reverse[word.length] == null)
                reverse[word.length] = Trie(0)
            reverse[word.length]!!.insert(word.reversed())
        }
        for (i in queries.indices){
            ...
            if (query.isEmpty()) answer[i] = roots[len]?.cnt ?: 0
                ...
                answer[i] = reverse[len]?.getCount(query.reversed()) ?: 0
        }
        return answer
    }
    ...
    data class Trie(
        var cnt : Int,
        val node : MutableMap<Char,Trie> = mutableMapOf()
    ){
