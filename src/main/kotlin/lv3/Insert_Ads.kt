...
class Insert_Ads {
    fun solution(play_time: String, adv_time: String, logs: Array<String>): String {
        val playT = toSec(play_time)
        val cnt = IntArray(playT){0}
        val advT = toSec(adv_time)

        for (log in logs){
            val l = log.split("-")
            val start = toSec(l[0])
            val end = toSec(l[1])
            for (i in start until end) cnt[i]++
        }
        ...
        for (i in cnt.indices){
            if (i<advT) sum += cnt[i]
            else sum = sum - cnt[i-advT]+cnt[i]
            if (sum > max){
                max = sum
                if (i >= advT)  startT = i - advT + 1
            }
        }
        return toStr(startT)
    }
    private fun toStr(sec : Int) : String{
        var tmp = sec
        val s = tmp % 60
        tmp /= 60
        val m = tmp % 60
        ...
        val h = tmp
        var str = ""
        str += if (h > 9) "$h:" else "0$h:"
        str += if (m > 9) "$m:" else "0$m:"
        str += if (s > 9) "$s" else "0$s"
        return str
    }
    private fun toSec(time : String) : Int{
        val t = time.split(":")
        return t[0].toInt()*60*60 + t[1].toInt()*60 + t[2].toInt()
    }
}