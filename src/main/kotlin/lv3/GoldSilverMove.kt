...
class GoldSilverMove {
    fun solution(a: Int, b: Int, g: IntArray, s: IntArray, w: IntArray, t: IntArray): Long {
        var start = 0L
        var end = 400000000000000L
        while (start < end){
            val mid = (start + end)/2
            var (gMax,sMax,tMax) = longArrayOf(0,0,0)
            for (i in s.indices){
                val gs : Long = g[i].toLong() + s[i]
                val cnt : Long = ((mid / t[i]) + 1 )/ 2
                val total = w[i] * cnt
                gMax += minOf(g[i].toLong(),total)
                sMax += minOf(s[i].toLong(),total)
                tMax += minOf(gs,total)
            }
            if (a <= gMax && b <= sMax && a + b <= tMax){
                end = mid
            }else{
                start = mid + 1
            }
