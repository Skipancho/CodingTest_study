...
class PyramidScheme {
    ...
    private val money = HashMap<String,Int>()
    ...
    fun solution(enroll: Array<String>, referral: Array<String>, seller: Array<String>, amount: IntArray): IntArray {
        ...
        for (j in seller.indices){
            count(seller[j],amount[j]*100)
        }
        return IntArray(enroll.size){i -> money[enroll[i]]!!}
    }

    private fun count(name : String, profit : Int){
        money[name] = money[name]!! + profit - (profit/10)
        if (map[name] == "-"){
            return
        }else{
            count(map[name]!!,profit/10)
        }
    }
}