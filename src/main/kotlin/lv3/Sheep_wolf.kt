...
class Sheep_wolf {
    ...
    fun bfs(tree : Array<IntArray> , info : IntArray) : Int{
        ...
        val visited = Array<Array<Boolean>>(17){Array<Boolean>(1.shl(17)){false}}
        queue.add(Node(0,1,0,1))
        visited[0][1] = true
        var max = 0

        while (!queue.isEmpty()){
            val node = queue.poll()
            max = max.coerceAtLeast(node.sheep)

            for (next in tree[node.id]){
                var nextS = 0
                var nextW = 0
                if (info[next]==0 && (node.state.and(1.shl(next)) ==0))
                    nextS = 1
                if (info[next]==1){
                    if (node.state.and(1.shl(next))==0){
                        nextW = 1
                    }
                    if (node.sheep <= node.wolf+nextW)
                        continue
                }
                val state = node.state.or(1.shl(next))
                if (visited[next][state])
                    continue
                visited[next][state] = true
                queue.add(Node(next,node.sheep+nextS,node.wolf+nextW,state))
            }
        }
