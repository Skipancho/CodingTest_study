...
class MatchingPoint {
    fun solution(word: String, pages: Array<String>): Int {
        var answer = 0
        var max = 0.0
        val map = hashMapOf<String,Page>()
        val word_low = word.toLowerCase()
        for( i in pages.indices){
            val p = Page(i)
            val page = pages[i].toLowerCase()
            val meta = "<meta property.+ content=\"https://.*\"/>".toRegex().find(page)!!.value
            val curUrl = "\"https://.*\"".toRegex().find(meta)!!.value
            val aTag  = "<a href=\"https://.\\S*\">".toRegex().findAll(page)!!.map { it.value }
            aTag.forEach {
                val link = "\"https://.*\"".toRegex().find(it)!!.value
                p.links.add(link)
            }
            val words = "[a-z]+".toRegex().findAll(page)!!.map { it.value }
            words.forEach {
                if (it == word_low){
                    p.count++
                    p.score += 1
        ...
        for (page in map.values){
            page.links.forEach {
                if (map.containsKey(it)){
                    val linkPage = map[it]!!
                    linkPage.score += page.count / page.links.size.toDouble()
                }
            }
        }
        for (page in map.values){
            if (max < page.score||(max == page.score && answer > page.idx)){
                max = page.score
                answer = page.idx
            }
        }
    ...
    data class Page(
        val idx : Int,
        val links : HashSet<String> = hashSetOf(),
        var count : Int = 0,
