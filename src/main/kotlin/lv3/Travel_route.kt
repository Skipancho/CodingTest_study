...
class Travel_route {
    ...
    private fun dfs(pos : Ticket, t : MutableList<Ticket>, route:MutableList<String>,n : Int){
        val tickets = t.map { it }.toMutableList()
        val newRoute = route.map { it }.toMutableList()
        if (newRoute.isNotEmpty())
            tickets.remove(pos)
        newRoute.add(pos.destination)
        ...
        map[cur]?.forEach {
            if (tickets.contains(Ticket(cur,it))){
                dfs(Ticket(cur,it),tickets,newRoute, n)
            }
        }
    }
