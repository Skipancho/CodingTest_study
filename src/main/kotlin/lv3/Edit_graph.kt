...
class Edit_graph {
    ...
    fun solution(n: Int, k: Int, cmd: Array<String>): String {
        ...
        nodes[0] = Node(0,false,null,null)
        for (i in 1 until n) {
            nodes[i] = Node(i,false,nodes[i-1],null)
            nodes[i-1]?.apply { this.next = nodes[i] }
        }
        var c_node = nodes[k]!!
        for (cm in cmd){
            val c = cm.split(" ")
            when(c[0]){
                ...
                "Z" -> {
                    if (stack.isNotEmpty()){
                        val removedNode = stack.pop()
                        removedNode.removed = false
